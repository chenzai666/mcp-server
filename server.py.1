...
def search_duckduckgo_html(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """DuckDuckGo HTML 兜底搜索，无需 API Key。"""
    response = requests.get(
        ...
        results.append(
            {
                ...
                "snippet": "; ".join(item.get("author", [{}])[0].get(k, "") for k in ["given", "family"] if item.get("author")) if item.get("author") else "",
                "source": "crossref",
            }
        )
    return results
...
def web_search(query: str, max_results: int = 5) -> str:
    """统一联网搜索。优先顺序由 SEARCH_BACKENDS 环境变量决定，已接入 Tavily。"""
    try:
...
def image_describe(image_url: str, prompt: str = "请描述这张图片的主要内容") -> str:
    ...
    try:
        ...
        # 没有视觉服务时，至少返回图片基本信息和 OCR 结果，避免工具完全不可用。
        r = _request(image_url, stream=True)
        r.raise_for_status()
        img = Image.open(io.BytesIO(r.content))
